...
insert /*+ append parallel(4) */
  into activity_swap_nwis (data_source_id, data_source, station_id, site_id, event_date, activity, sample_media,
                           ...
                           act_sam_transport_storage_desc, deprecated_flag)
select 1 data_source_id,
       ...
       case
         when samp.sample_start_dt is not null and samp.sample_start_sg in ('h','m') then samp.local_tz_cd
         else null
       ...
       case 
         when samp.sample_end_dt is not null and samp.sample_end_sg in ('h', 'm') then samp.local_tz_cd
         else null
...
--         on samp.site_id = site.site_id
       join station_swap_nwis s
         on samp.agency_cd || '-' || samp.site_no = s.site_id
...
--            site.nwis_host = dist.host_name
       ...
       left join nwis_ws_star.nawqa_sites
         on samp.site_no = nawqa_sites.site_no and
            samp.agency_cd = nawqa_sites.agency_cd;
